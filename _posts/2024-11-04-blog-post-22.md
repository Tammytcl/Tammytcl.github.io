...
$$\\mathbf{\mathit{r}}=\\mathbf{\mathit{f}}(t),\quad t\in[\alpha,\beta]$$

### 向量函数定义
设数集 $D\subset \\mathbf{R}$，则称映射 $\\mathbf{\mathit{f}}:\mathit{D}\rightarrow\\mathbf{R}^{n}$ 为一元向量函数，通常记为

$$\\mathbf{\mathit{r}}=\\mathbf{\mathit{f}}(t),\quad t\in \mathit{D}$$

其中数集 $\mathit{D}$ 称为函数值的定义域，$\\mathbf{\mathit{t}}$ 称为自变量，$\\mathbf{\mathit{r}}$ 称为因变量。

### 向量函数极限定义
设向量函数 $\\mathbf{\mathit{f}}(t)$ 在点 $t_{0}$ 的某一去心邻域内有定义。如果存在一个常向量 $\\mathbf{\mathit{r}}_{0}$，对任意给定的正数 $\epsilon$，总存在正数 $\delta$，使得当 $\mathit{t}$ 满足 $0<\left|t-t_{0}\right|<\delta$ 时，对应的函数值 $\\mathbf{\mathit{f}}(t)$ 都满足不等式

$$\left|\\mathbf{\mathit{f}}(t)-\\mathbf{\mathit{r}}_{0}\right|<\epsilon$$

那么，常向量 $\\mathbf{\mathit{r}}_{0}$ 叫做向量函数 $\\mathbf{\mathit{f}}(t)$ 当 $t\rightarrow t_{0}$ 时的极限，记作

$$\lim\limits_{t\rightarrow t_{0}}\\mathbf{\mathit{f}}(t)=\\mathbf{\mathit{r}}_{0}$$

### 向量函数导数的定义

设设向量函数 $\\mathbf{\mathit{r}}=\\mathbf{\mathit{f}}(t)$ 在点 $t_{0}$ 的某一去心邻域内有定义。如果

$$\lim\limits_{\Delta t\rightarrow0}\dfrac{\Delta \\mathbf{r\mathit{}}}{\Delta t}=\lim\limits_{\Delta t\rightarrow0}\dfrac{\\mathbf{\mathit{f}}(t_{0}+\Delta t)-\\mathbf{\mathit{f}}(t_{0})}{\Delta t}$$

存在，那么就称这个极限向量为向量函数 $\\mathbf{\mathit{r}}=\\mathbf{\mathit{f}}(t)$ 在点 $t_{0}$ 处的导数或导向量，记作 $\\mathbf{\mathit{f}}'(t_{0})$
...
导向量 $\\mathbf{\mathit{T}}=\\mathbf{\mathit{f}}'(t_{0})=(\varphi'(t_{0}),\psi'(t_{0}),\omega'(t_{0}))$ 就是曲线 $\tau$ 在点 $\\mathbf{\mathit{M}}$ 处的一个切向量，从而我们可以得到点 $\\mathbf{\mathit{M}}$ 处的切线方程为

$$\dfrac{x-x_{0}}{\varphi'(t_{0})}=\dfrac{y-y_{0}}{\psi'(t_{0})}=\dfrac{z-z_{0}}{\omega'(t_{0})}$$

过点 $\\mathbf{\mathit{M}}$ 且与切线垂直的平面称为曲线 $\tau$ 在点 $\\mathbf{\mathit{M}}$ 处的法平面，它是通过点 $M(x_{0},y_{0},z_{0})$ 且以 $\\mathbf{\mathit{T}}=\\mathbf{\mathit{f}}'(t_{0})$为 法向量的平面，因此根据我们先前所学的，我们可以得到法平面方程
...
$\\mathbf{\mathit{n}}=(F_{x}(x_{0},y_{0},z_{0}),F_{y}(x_{0},y_{0},z_{0}),F_{z}(x_{0},y_{0},z_{0}))$为切平面的法向量。$T=(\varphi'(t_{0}),\psi'(t_{0}),\omega'(t_{0}))$为我们选的这条曲线的切向量。
