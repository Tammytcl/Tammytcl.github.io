...
设 $\boldsymbol{a}$ 与 $\boldsymbol{b}$ 为两个非零向量，$<\boldsymbol{a}, \boldsymbol{b}>$ 表示它们之间的夹角，范围在 $[0, \pi]$ 内。向量 $\boldsymbol{a}$ 与 $\boldsymbol{b}$ 的内积定义为：
...
\[
\boldsymbol{u} \times \boldsymbol{v} = \left( |\boldsymbol{u}| \cdot |\boldsymbol{v}| \sin \theta \right) \boldsymbol{n}
\]

\[
|\boldsymbol{u} \times \boldsymbol{v}| = |\boldsymbol{u}| \cdot |\boldsymbol{v}| \sin \theta
\]
...
- 极坐标系表示下：
  \[
  \boldsymbol{a} = |\boldsymbol{a}|\boldsymbol{\hat{a}}
  \]
...
\]
\[
\boldsymbol{a} \times \boldsymbol{b} = 0 \rightarrow \boldsymbol{a} \parallel \boldsymbol{b}
\]

向量 $\boldsymbol{a}$ 和 $\boldsymbol{b}$ 的叉乘结果相当于图像中平行四边形的面积大小。
...
如图所示，叉乘结果是沿 $\overrightarrow{OC_1}$ 方向的一个向量，其大小为平行四边形 OABD 的面积大小，再与 $\boldsymbol{c}$ 做点乘，相当于再乘以 $\boldsymbol{c}$ 在 $\overrightarrow{OC}$ 方向的投影，代表了平行六面体的体积。
...
如果我们要求平面外一点到平面内的距离，我们可以先找到经过平面外该点的法向量，再找平面上一点$P_{1}$,距离\textit{d}就是$\overrightarrow{P_{1}P}$在法向量上的投影。
$$
d = \left|\overrightarrow{P_{1}P}\right|\left|\cos\theta\right| = \frac{\left|\overrightarrow{P_{1}P}\cdot \boldsymbol{n}\right|}{\left|\boldsymbol{n}\right|} = \frac{\left|Ax_{0} + By_{0} + Cz_{0} + D\right|}{\sqrt{A^{2} + B^{2} + C^{2}}}
$$
...
绕\textit{z}轴旋转我们得到旋转单叶双曲面,绕$x$轴旋转我们能得到旋转双叶双曲面
$$
