...
我们已知两个方向向量 $\mathit{\mathbf{s}_{1}} = (m_{1}, n_{1}, p_{1})$ 和 $\mathit{\mathbf{s}_{2}}=(m_{2},n_{2},p_{2})$
...
### 向量函数极限定义
设向量函数 $\mathit{\mathbf{f}}(t)$ 在点 $t_{0}$ 的某一去心邻域内有定义。如果存在一个常向量 $\mathit{\mathbf{r}}_{0}$，对任意给定的正数 $\epsilon$，总存在正数 $\delta$，使得当 $\mathit{t}$ 满足 $0<\left|t-t_{0}\right|<\delta$ 时，对应的函数值 $\mathit{\mathbf{f}}(t)$ 都满足不等式

$$\left|\mathit{\mathbf{f}}(t)-\mathit{\mathbf{r}}_{0}\right|<\epsilon$$

那么，常向量 $\mathit{\mathbf{r}}_{0}$ 叫做向量函数 $\mathit{\mathbf{f}}(t)$ 当 $t\rightarrow t_{0}$ 时的极限，记作

$$\lim\limits_{t\rightarrow t_{0}}\mathit{\mathbf{f}}(t)=\mathit{\mathbf{r}}_{0}$$
...
过点 $\mathit{\mathbf{M}}$ 且与切线垂直的平面称为曲线 $\tau$ 在点 $\mathbit\mathbf{M}}$ 处的法平面，它是通过点 $M(x_{0},y_{0},z_{0})$ 且以 $\mathit{\mathbf{T}}=\mathit{\mathbf{f}}'(t_{0})$为 法向量的平面，因此根据我们先前所学的，我们可以得到法平面方程
