...
### 向量函数极限定义
设向量函数 $\mathbf{f}(t)$ 在点 $t_{0}$ 的某一去心邻域内有定义。如果存在一个常向量 $\mathbf{r_{0}}$，对任意给定的正数 $\epsilon$，总存在正数 $\delta$，使得当 $t$ 满足 $0<\left|t-t_{0}\right|<\delta$ 时，对应的函数值 $\mathbf{f}(t)$ 都满足不等式

$$\left|\mathbf{f}(t)-\mathbf{r}_{0}\right|<\epsilon$$
