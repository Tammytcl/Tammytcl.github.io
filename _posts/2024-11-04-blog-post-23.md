...
$$
\left|a_{n}-a_{m}\right|<\epsilon\qquad\mbox{只要}n\geqslant N,m\geqslant N
$$
...
$$
0<\left|x_1-a\right|<\delta\qquad 0<\left|x_2-a\right|<\delta
$$
...
$$
\sum_{k=n+1}^{k=n+p}&\frac{1}{k^2}=\frac{1}{(n+1)^2}+\frac{1}{(n+2)^2}+\cdots+\frac{1}{(n+p)^2}
...
注意达朗贝尔判别法相比定理2是自身级数前后项做比值，其实这个类似于高中学到等比数列压塑，前后项比值为 \textit{l}，我们可以放缩成公比为\ textit{l} 的等比数列，再对等比数列求极限。
...
若其各项取绝对值后所形成的正项级数 $\sum\limits_{n=1}^{\infty}\left|u_n\right|$ 收敛，则级数 $\sum\limits_{n=1}^{\infty}u_n$ 收敛。

## 定义
若正项级数 $\sum\limits_{n=1}^{\infty}\left|u_n\right|$ 收敛，则称 $\sum\limits_{n=1}^{\infty}u_n$ 绝对收敛。若级数 $\sum\limits_{n=1}^{\infty}u_n$ 收敛但 $\sum\limits_{n=1}^{\infty}\left|u_n\right|$ 发散，则称级数 $\sum\limits_{n=1}^{\infty}u_n$ 条件收敛.

对于一般的级数 $\sum\limits_{n=1}^{\infty}u_n$，我们先用正项级数的判别法来判别级数$\sum\limits_{n=1}^{\infty}\left|u_n\right|$是否收敛，若收敛，则 $\sum\limits_{n=1}^{\infty}u_n$ 必收敛。
...
- 当$\left|x\right|<R$时，级数$\sum\limits_{n=0}^{\infty}a_{n}x^n$绝对收敛。 
- 当$\left|x\right|>R$时，级数$\sum\limits_{n=0}^{\infty}a_{n}x^n$发散。 
- 当$\left|x\right|=R$时，级数$\sum\limits_{n=0}^{\infty}a_{n}x^n$可能收敛也可能发散。
...
$$
\frac{\left|a_{n+1}x^{n+1}\right|}{\left|a_{n}x^{n}\right|}=\left|\frac{a_{n+1}}{a_{n}}\right|
\left|x\right|
$$

如果
$
\lim\limits_{n\rightarrow\infty}\left|\dfrac{a_{n+1}}{a_n}\right|=\rho
$ 存在，根据比值审敛法，当 $\rho \left|x\right|<1$ 即 $\left|x\right|<\dfrac{1}{\rho}$ 时，级数发散。

所以当 \textit{x} 上面的次数是 $2n$ 而不是 $n$ 时，相邻两项相差2位，$\rho\left|x^2\right|>1$，所以最后的结果需要开根号。
...
$$
(\sum\limits_{n=0}^{\infty}a_{n}x^n)\cdot*(\sum\limits_{n=0}^{\infty}b_{n}x^n)=&a_{0}b_{0}+(a_{0}b_{1}+a_{1}b_{0})+\cdots\\
...
当 $b_0\neq0$，且 $\left|x\right|$ 充分小时，两幂级数 $\sum\limits_{n=0}^{\infty}a_{n}x^n$ 与 $\sum\limits_{n=0}^{\infty}b_{n}x^n$
可相除，它们的商也是幂级数。
...
$$
\left|R_{n}(x)\right|=\left|\frac{e^{\theta x}}{(n+1)!}x^{n+1}\right|\leq\frac{e^{\left|x\right| }}{(n+1)!}\left|x\right|^{n+1}
$$

对任意取定的$x\in(-\infty,+\infty),\dfrac{e^{\left|x\right| }}{(n+1)!}\left|x\right|^{n+1}\rightarrow0(n\rightarrow\infty)$，因而$R_{n}(x)\rightarrow0(n\rightarrow\infty)$
...
$$
\left|R_{2n}(x)\right|\leq \dfrac{1}{(2n+1)!}\left|x\right|^{2n+1}\rightarrow0(n\rightarrow\infty)
$$
...
&=\frac{a_0}{2}	\int_{-\pi}^{\pi}\sin kx \mathrm{d}x+	\int_{-\pi}^{\pi}\sum\limits_{n=1}^{\infty}(a_n\cos nx\sin kx+b_n\sin nx\sin kx)\mathrm{d}x\\
&=\frac{a_0}{2}	\int_{-\pi}^{\pi}\sin kx \mathrm{d}x+\sum\limits_{n=1}^{\infty}\big(a_n\int_{-\pi}^{\pi}\cos nx\sin kx\mathrm{d}x+b_n\int_{-\pi}^{\pi}\sin nx\sin kx\mathrm{d}x\big)
$$
