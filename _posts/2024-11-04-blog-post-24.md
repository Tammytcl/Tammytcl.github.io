...
$$\textbf{\  $ \textit{r} $  }^{2}= \left\{(x,y)|x\in \textbf{\  $ \textit{r} $  }, y\in \textbf{R}\right\}$$
...
在数轴  $ \textbf{\  $ \textit{r} $  } $  上点  $ x $  到  $ x_{0} $  的距离为：
 $ \left|x-x_{0}\right| = \sqrt{(x-x_{0})^{2}} $ 
...
\paragraph{定义1}设函数 $ z=f(x,y) $ 在点 $ (x_{0},y_{0}) $ 的某个空心邻域内有定义，若有一常数\textit{A}，对任意给定的正数 $ \epsilon $ ,都有正数 $ \delta $ 使得当
\begin{align}
0<\sqrt{(x-x_{0})^{2} + (y-y_{0})^{2}}<\delta
\end{align}
时，就有 $ \left|f(x,y)-A\right|<\epsilon $ 则称 $ (x,y) $ 趋于 $ (x_{0},y_{0}) $ 时， $ f(x,y) $ 以\textit{A}为极限，
...
\begin{align*}
\left|x-x_{0}\right|<\epsilon,\quad\left|y-y_{0}\right|<\epsilon,\mbox{且}(x,y)\ne (x_{0},y_{0})
\end{align*}
这个式子和欧氏距离的定义是等价的。
\paragraph{定义2}设函数 $ f(x,y) $ 在 $ (x_{0},y_{0}) $ 的一个空心邻域内有定义，若存在一个常数\textit{A}，对于任意的 $ \epsilon>0 $ ，都存在一个 $ \delta>0 $ ，使得当
\begin{align*}
\left|x-x_{0}\right|<\epsilon,\quad\left|y-y_{0}\right|<\epsilon,\mbox{且}(x,y)\ne (x_{0},y_{0})
\end{align*}
时，便有
$$\left|f(x,y)-A\right|<\epsilon$$
...
## 多元函数的性质
\paragraph{连续的定义}设函数 $ u=f(x,y) $ 在点 $ (x_{0},y_{0}) $ 的一个邻域内有定义，若函数 $ u=f(x,y) $ 当 $ (x,y)\rightarrow(x_{0},y_{0}) $ 时有极限，且其极限等于函数值 $ f(x_{0},y_{0}) $ 也即
\begin{align}
...
\paragraph{有界性定义}设函数 $ f(P) $ 在有界闭区间 $ \bar{D} $ 上连续，则 $ f(P) $ 在 $ \bar{D} $ 上有界，即存在常数 $ \textbf{\textit{M}}>0 $ ，使
$$\left|f(P)\right|\leq \textbf{\textit{M}}\quad \forall P\in\bar{D}$$

\paragraph{最大值最小值定理}设函数 $ f(P) $ 在有界闭区间 $ \bar{D} $ 上连续，则 $ f(P) $ 在 $ \bar{D} $ 上取到最大值最小值，即存在点 $ P_{1},P_{2}\in \bar{D} $ ，使
$$f(P)\leq f(P_{1}), \quad f(P_{2})\leq f(P)\quad \forall P\in\bar{D}$$

\paragraph{介值定理}设函数 $ f(P) $ 在有界闭区间 $ \bar{D} $ 上连续，并假定\textbf{\textit{M}}与\textbf{\textit{m}}分别是 $ f(P) $ 在 $ \bar{D} $ 的最大值和最小值，则对于任意的 $ \eta:\quad \textbf{\textit{m}}\leq \eta\leq\textbf{\textit{M}} $ ，一定有一点
...
\begin{figure}[hbt!]
\centering
\includegraphics[width=0.5\textwidth]{picture/11.png}
\end{figure}

通过图像我们能明白固定其他变量的意义，那么我们引出偏导数的定义

\paragraph{偏导数定义}设函数 $ z=f(x,y) $ 在点 $ (x_{0},y_{0}) $ 的某一个邻域内有定义，当\textit{y}固定在 $ y_{0} $ ，而\textit{x}在 $ x_{0} $ 处有增量 $ \Delta x $ 时，相应的函数有增量
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.4\textwidth]{picture/14.png}
\end{figure}
...
&=\textbf{grad}f(x_{0},y_{0})\textbf{  $ \textit{E} $   }=\left|\textbf{grad}f(x_{0},y_{0})\right| \cos\theta
\end{align}
$$
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.8\textwidth]{picture/15.png}
\end{figure}
...
\begin{figure}[H]
\centering{\includegraphics[width=7cm]  {picture/17.png}}
\end{figure}
...
\left|	\iint\limits_{D}f(x,y)\mathrm{d}\sigma\right|&\leq \iint\limits_{D}\left|f(x,y)\right|\mathrm{d}\sigma
\end{align*}
$$
...
我们用数学语言来构建极坐标系
\begin{figure}[H]
\centering{\includegraphics[width=7cm]  {picture/16.jpg}}
\end{figure}
...
\begin{figure}[H]
\centering{\includegraphics[width=7cm]  {picture/18.png}}
\end{figure}
...
\begin{figure}[H]
\centering{\includegraphics[width=5cm]  {picture/19.png}}
\end{figure}
...
## 柱坐标系
\begin{figure}[H]
\centering{\includegraphics[width=7cm]  {picture/20.png}}
\end{figure}
...
\begin{figure}[H]
\centering{\includegraphics[width=5cm]  {picture/21.png}}
\end{figure}
...
## 球坐标系
\begin{figure}[H]
\centering{\includegraphics[width=8cm]  {picture/22.png}}
\end{figure}
...
\begin{figure}[H]
\centering{\includegraphics[width=7cm]  {picture/23.png}}
\end{figure}
...
为了求得曲面\textit{S}的面积，我们考虑曲面上任意一点 $ P(x,y,z) $ ，并考虑增量d\textit{x}>0及d\textit{y}>0，记 $ \Delta \sigma=\mathrm{d}x\mathrm{d}y $ 
并记 $ \Delta S $ 为对应于这两个增量曲面上微元的面积

\begin{figure}[H]
\centering{\includegraphics[width=7cm]  {picture/24.png}}
\end{figure}

过 $\textit{P}$ 点作 $\textit{S}$ 的切平面，对应微元 $ \Delta \sigma $ 切平面的微元的面积我们记作 $ \Delta S' $ ，我们有

$$\Delta \sigma = \left|\cos\gamma\right|\Delta S'$$

其中 $ \gamma $ 是曲面 $\textit{S}$ 在\textit{P}点的法向量 $\textbf{  $ \textit{n} $   }$ 与 $\textit{z}$ 轴的夹角

我们可以表示在\textit{P}点的法向量 $ n=((f_{x}(x,y)),f_{y}(x,y),-1) $ ，因此

$$\left|\cos\gamma\right|=\frac{1}{\sqrt{1+f_{x}^{2}(x,y)+f_{y}^{2}(x,y)}}$$
...
\begin{figure}[H]
\centering{\includegraphics[width=5cm]  {picture/25.png}}
\end{figure}
...
\begin{figure}[H]
\centering{\includegraphics[width=7cm]  {picture/26.png}}
\end{figure}
...
\begin{figure}[H]
\centering{\includegraphics[width=7cm]  {picture/27.png}}
\end{figure}
...
$$
\mathrm{rot}\textbf{\textit{F}}\cdot \textbf{  $ \textit{n} $   }=\left|\mathrm{rot}\textbf{\textit{F}}\right|\cos<\textbf{  $ \textit{n} $   },\mathrm{rot}\textbf{\textit{F}}>
$$
