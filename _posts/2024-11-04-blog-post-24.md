...
### 定义
设 $ z=f(x,y) $ 是定义在有界闭区域 $\textit{D}$ 上的函数，若对 $\textit{D}$ 的任意分割{ $ D_{1},D_{2},\cdots, D_{n}, $ }及任意选择的 $ (x_{i},y_{i})\in D_{i}(i=1,2,\cdots ,n) $ ， $ \lambda $ 表示 $ D_{i} $ 中直径的最大者，当 $ \lambda\rightarrow0 $ 时，黎曼和 $ \underset{i=1}{\overbrace{n}{\sum}}f(x_{i},y_{i})\Delta\sigma_{i} $ 总有极限
...
若极限 $ \lim\limits_{\lambda\rightarrow 0}\underset{i=1}{\overbrace{n}{\sum}}f(\xi_{i},\eta_{i},\zeta_{i})\Delta s_{i} $ ，对于曲线 $\textit{L}$ 的任意分割法及中间点 $ (\xi_{i},\eta_{i},\zeta_{i}) $ 的任意取法都存在，则称此极限为函数 $ f(x,y,z) $ 沿曲线 $\textit{L}$ 的第一类曲线积分，也叫做对弧长的曲线积分，记作
...
$$\int_{\overbrace{AB}}f(m)\mathrm{d}s=\int_{\overbrace{BA}}f(m)\mathrm{d}s$$

### 计算
我们先讨论平面上的第一类曲线积分的计算，假设 $\textit{L}$ 是 $\textit{Oxy}$ ，其方程由函数 $ y=y(x) (a\leq x \leq b) $ ，由设 $ f(x,y) $ 是 $\textit{L}$ 上的一个连续函数，现在问题是如何计算

$$I=\int_{L}f(x,y)\mathrm{d}s$$我们这里的根据定义出发这个积分是和式 $ \underset{i=1}{\overbrace{m}{\sum}}f(\xi_{i},\eta_{i})\Delta s_{i} $ 的极限，我们在二维平面里， $ \eta $ 实际上是对 $\textit{L}$ 的一个分割，是与 $\mathbf{\textit{r}}$ 轴坐标相关的参数，我们可以写为
...
我们用分点 $ A=A_{0},A_{1},\cdots ,A_{n}=B $ 将曲线 $ \overbrace{AB} $ 任意分割成 $ \textit{n} $ 段小弧

$$\overbrace{A_{i-1}A_{i}}\quad(i=1,2,\cdots,n)$$

设第i段的弧长为 $ \Delta s_{i} $ ，当 $ \Delta s_{i} $ 很小时，\mathit{\mathbf{F}}在 $ \overbrace{A_{i-1}A_{i}} $ 上的变化不大，可近似的看作为常力 $ \mathit{\mathbf{F}}(\xi_{i},\eta_{i}) $ ，其中 $ (\xi_{i},\eta_{i}) $ 为弧段 $ \overbrace{A_{i-1}A_{i}} $ 上任取的一点，同时可将指点的运动轨迹 $ \overbrace{A_{i-1}A_{i}} $ 近似的看作从 $ A_{i-1} $ 到 $ A_{i} $ 的一条直线，于是\mathit{\mathbf{F}}在这段上做的功 $ \Delta W_{i} $ ，近似为
...
令 $ \lambda = \max\limits_{1\leq i \leq n}{\Delta s_{i}} $ ，我们定义变力\mathit{\mathbf{F}}沿 $ \overbrace{AB} $ 所做的功为
...
$$\int_{\overbrace{AB}}P\mathrm{d}x+Q\mathrm{d}y \quad or \quad  \int_{\overbrace{AB}}\mathit{\mathbf{F}}(x,y)\cdot \mathrm{d}\mathit{\mathbf{r}}$$

其中 $ \mathrm{d}\mathit{\mathbf{r}}=(\mathrm{d}x,\mathrm{d}y) $ ，有向线段 $ \overbrace{AB} $ 称为积分路径

由定义看出，外力 $\textit{F}$ 沿曲线 $\textit{L}$ 从 $\textit{A}$ 到 $ $\textit{B}$ $ 所做的功可表示为第二类曲线积分

$$W=\int_{\overbrace{AB}}\mathit{\mathbf{F}}(x,y)\cdot \mathrm{d}\mathit{\mathbf{r}}$$
...
$$
\begin{align*}
\int_{\overbrace{AB}}P(x,y)\mathrm{d}x+Q(x,y)\mathrm{d}y=
...
$$\oint_{\overbrace{AB}}P(x,y)\mathrm{d}x+Q(x,y)\mathrm{d}y$$

的值只与起点 $\textit{A}$，终点 $\textit{B}$ 的坐标有关，而与连接 $\textit{A,B}$ 两点的积分路径无关。

### 命题
在区域 $\textit{D}$ 内任意选定两点 $\textit{A,B}$，曲线积分 $ \oint_{\overbrace{AB}}P(x,y)\mathrm{d}x+Q(x,y)\mathrm{d}y $ 在区域 $\textit{D}$ 内与路径无关的充分必要条件是：对于 $\textit{D}$ 内任意一条简单逐段光滑的闭合曲线 $\textit{C}$，沿 $\textit{C}$ 的曲线积分为零，即
...
根据这张图，我们可以把闭合曲线 $\textit{C}$ 分为 $ \overbrace{AnB} $ 和 $ \overbrace{AmB} $ 

$$\oint_{\overbrace{AnB}}P(x,y)\mathrm{d}x+Q(x,y)\mathrm{d}y=\oint_{\overbrace{AmB}}P(x,y)\mathrm{d}x+Q(x,y)\mathrm{d}y$$

移项得

$$
\begin{align*}
0=&\oint_{\overbrace{AmB}}P(x,y)\mathrm{d}x+Q(x,y)\mathrm{d}y-\oint_{\overbrace{AnB}}P(x,y)\mathrm{d}x+Q(x,y)\mathrm{d}y\\
=&\oint_{\overbrace{AmB}}P(x,y)\mathrm{d}x+Q(x,y)\mathrm{d}y+\oint_{\overbrace{BnA}}P(x,y)\mathrm{d}x+Q(x,y)\mathrm{d}y\\
=&\oint_{C^{+}}Pdx+Qdy
\end{align*}
$$
...
设函数 $ P(x,y),Q(x,y) $ 在单连通区域 $\textit{D}$ 内有连续的一阶偏导数，对任意两点 $ A,B\in D $ ，曲线积分 $ \oint_{\overbrace{AB}}P(x,y)\mathrm{d}x+Q(x,y)\mathrm{d}y $ 与路径无关的充要条件是： $ P(x,y)\mathrm{d}x+Q(x,y)\mathrm{d}y $ 恰是某个函数 $ u(x,y) $ 的全微分，此外，当 $ P(x,y)\mathrm{d}x+Q(x,y)\mathrm{d}y $ 是 $ u(x,y) $ 的全微分时，有

$$\oint_{\overbrace{AB}}P(x,y)\mathrm{d}x+Q(x,y)\mathrm{d}y=\int_{A}^{B}\mathrm{d}u=u(B)-u(A)$$
...
在物理学中，我们定义了电场线和磁场线，我们在数学中定义，如果在向量场 $\mathit{\mathbf{F}}$ 中，若曲线 $ \Gamma $ 上每一点 $\textit{M}$ 处的切线都与向量场\mathit{\mathbf{F}}在 $\textit{M}$ 点的向量 $ \mathit{\mathbf{F}}\bigg|_{\substack{M}} $ 共线，则 $ \Gamma $ 被称为向量线。
...
$$
\oiint\limits_{S}\mathit{\mathbf{F}}\cdot \mathit{\mathbf{n}}\mathrm{d}S=
...
$$
\oiint\limits_{S}\mathit{\mathbf{F}}\cdot \mathit{\mathbf{n}}\mathrm{d}S=
\iiint\limits_{V}\mathrm{div}\mathit{\mathbf{F}}\mathrm{d}v
$$

由积分中值定理我们可以得到

$$
\iiint\limits_{V}\mathrm{d}{div}\mathit{\mathbf{F}}\mathrm{d}v=
\mathrm{div}\mathit{\mathbf{F}}\bigg|_{\substack{(\bar{x},\bar{y},\bar{z})}}\cdot m(V)
$$

其中 $ m(V) $ 表示的\textit{V}的体积，这样当积分区域V缩小到一点 $ M_{0}(x_{0},y_{0},z_{0}) $ 时，通量的平均值的极限即是 $ \mathrm{d}{div}\mathit{\mathbf{F}} $ 在 $ M_{0} $ 的值

$$
\mathrm{div}\mathit{\mathbf{F}}\bigg|_{\substack{(M_{0})}}=\lim\limits_{V\rightarrow M_{0}}\dfrac{\oiint\limits_{S}\mathit{\mathbf{F}}\cdot \mathit{\mathbf{n}}\mathrm{d}S}{m(V)}
$$
...
设 $ \mathit{\mathbf{F}}=P\mathit{\mathbf{i}}+Q\mathit{\mathbf{j}}+R\mathit{\mathbf{k}} $ 是定义在区域 $ D\subset R^3 $ 中的一个向量场，若沿 $\textit{D}$ 中的任意一条曲线 $ \overbrace{AB} $ 的积分

$$
\int_{\overbrace{AB}}\mathit{\mathbf{F}}\cdot \mathrm{d}\mathit{\mathbf{r}}=\int_{\overbrace{AB}}P\mathrm{d}x+Q\mathrm{d}y+R\mathrm{d}z
$$

只与曲线的起点\textit{A}和终点 $\textit{B}$ 有关，而与曲线 $ \overbrace{AB} $ 的路径无关，则称向量场\mathit{\mathbf{F}}在 $\textit{D}$ 内是一个保守场。
...
- $ \mathit{\mathbf{F}} $  在D内沿任意闭曲线 $ \Gamma $ 的环量为零，即 $ \int_{\overbrace{AB}}\mathit{\mathbf{F}}\cdot \mathrm{d}\mathit{\mathbf{r}}=0 $ \\
