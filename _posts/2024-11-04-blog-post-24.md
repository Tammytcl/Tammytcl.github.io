...
多元函数顾名思义就是多个变量的函数，我们生活常见的应用相关的问题也大多是多元函数的问题。我们在介绍一元函数的时候，我们一开始是通过映射去定义函数的，二元函数是两个变量在一定范围内变化时，相应的数组对应平面上某一点集合。那么同理我们可以知道三元函数的实质就是三维空间中某个点集合到实数 $ \mathit{\mathbf{r}}$ 的一个映射。

我们用数学语言来给出定义：
我们将全体实数组 $ (x,y) $  所组成的集合记作 $ \mathit{\mathbf{r}}^{2} $  即

$$
\mathbf{R}^{2} = \left\{(x,y) \mid x \in \mathit{\mathbf{R}}, y \in \mathit{\mathbf{R}}\right\}
$$

同理，全体实数组 $ (x,y,z) $ 所组成的集合记作 $ \mathit{\mathbf{R}}^{3} $，即

$$
\mathit{\mathbf{R}}^{3} = \left\{(x,y,z) \mid x \in \mathit{\mathbf{R}}, y \in \mathit{\mathbf{R}}, z \in \mathit{\mathbf{R}}\right\}
$$

对于更一般的 $ n $ 元函数，我们构建 $ n $ 个有次序的实数组： $ (x_{1}, x_{2}, \cdots, x_{n}) $，构成 $ \mathit{\mathbf{R}}^{n} $，即

$$
\mathit{\mathbf{R}}^{n} = \left\{(x_{1}, x_{2}, \cdots, x_{n}) \mid x_{j} \in \mathit{\mathbf{R}}, j=1, 2, \ldots, n\right\}
$$
...
故 $ y_{j} $  是 $ (x_{1},x_{2},\cdots,x_{n}) $  的一个函数，设它为 $ f_{j}(x_{1},x_{2},\cdots,x_{n})(j=1,2,\dots m) $  这样映射 $ f:D\rightarrow\mathit{\mathbf{r}}^{m} $  相当于 $\textit{M}$ 个  $ \textit{n} $  元函数
...
设 $ E\subset \mathit{\mathbf{r}}^{n} $  是一个给定的集合，根据集合 $\textit{E}$ 我们将
...
存在，那么称此极限为函数 $ z=f(x,y) $ 在点 $ (x_{0},y_{0}) $ 处对 $\mathbf{\textit{r}}$ 的偏导数。记作 $ \frac{\partial z}{\partial x} \mid_{\substack{x=x_0, y=y_0}},\frac{\partial f}{\partial x} \mid_{\substack{x=x_0, y=y_0}},z_{x}\mid_{\substack{x=x_0, y=y_0}}, $ 
...
记作 $ \frac{\partial z}{\partial y} \mid_{\substack{x=x_0\\y=y_0}},\frac{\partial f}{\partial y} \mid_{\substack{x=x_0\\y=y_0}},z_{y}\mid_{\substack{x=x_0\\y=y_0}},f_{y}(x_{0},y_{0}) $ 
...
### 定义
设 $ z=f(x,y) $ 是定义在有界闭区域 $\textit{D}$ 上的函数，若对 $\textit{D}$ 的任意分割{ $ D_{1},D_{2},\cdots, D_{n}, $ }及任意选择的 $ (x_{i},y_{i})\in D_{i}(i=1,2,\cdots ,n) $ ， $ \lambda $ 表示 $ D_{i} $ 中直径的最大者，当 $ \lambda\rightarrow0 $ 时，黎曼和 $ \underset{i=1}{\widehat{n}{\sum}}f(x_{i},y_{i})\Delta\sigma_{i} $ 总有极限
...
若极限 $ \lim\limits_{\lambda\rightarrow 0}\underset{i=1}{\widehat{n}{\sum}}f(\xi_{i},\eta_{i},\zeta_{i})\Delta s_{i} $ ，对于曲线 $\textit{L}$ 的任意分割法及中间点 $ (\xi_{i},\eta_{i},\zeta_{i}) $ 的任意取法都存在，则称此极限为函数 $ f(x,y,z) $ 沿曲线 $\textit{L}$ 的第一类曲线积分，也叫做对弧长的曲线积分，记作
...
$$\int_{\widehat{AB}}f(m)\mathrm{d}s=\int_{\widehat{BA}}f(m)\mathrm{d}s$$

### 计算
我们先讨论平面上的第一类曲线积分的计算，假设 $\textit{L}$ 是 $\textit{Oxy}$ ，其方程由函数 $ y=y(x) (a\leq x \leq b) $ ，由设 $ f(x,y) $ 是 $\textit{L}$ 上的一个连续函数，现在问题是如何计算

$$I=\int_{L}f(x,y)\mathrm{d}s$$我们这里的根据定义出发这个积分是和式 $ \underset{i=1}{\widehat{m}{\sum}}f(\xi_{i},\eta_{i})\Delta s_{i} $ 的极限，我们在二维平面里， $ \eta $ 实际上是对 $\textit{L}$ 的一个分割，是与 $\mathbf{\textit{r}}$ 轴坐标相关的参数，我们可以写为
...
我们用分点 $ A=A_{0},A_{1},\cdots ,A_{n}=B $ 将曲线 $ \widehat{AB} $ 任意分割成 $ \textit{n} $ 段小弧

$$\widehat{A_{i-1}A_{i}}\quad(i=1,2,\cdots,n)$$

设第i段的弧长为 $ \Delta s_{i} $ ，当 $ \Delta s_{i} $ 很小时，\mathit{\mathbf{F}}在 $ \widehat{A_{i-1}A_{i}} $ 上的变化不大，可近似的看作为常力 $ \mathit{\mathbf{F}}(\xi_{i},\eta_{i}) $ ，其中 $ (\xi_{i},\eta_{i}) $ 为弧段 $ \widehat{A_{i-1}A_{i}} $ 上任取的一点，同时可将指点的运动轨迹 $ \widehat{A_{i-1}A_{i}} $ 近似的看作从 $ A_{i-1} $ 到 $ A_{i} $ 的一条直线，于是 $\mathit{\mathbf{F}}$ 在这段上做的功 $ \Delta W_{i} $ ，近似为
...
令 $ \lambda = \max\limits_{1\leq i \leq n}{\Delta s_{i}} $ ，我们定义变力 $\mathit{\mathbf{F}}$ 沿 $ \widehat{AB} $ 所做的功为
...
$$\int_{\widehat{AB}}P\mathrm{d}x+Q\mathrm{d}y \quad or \quad  \int_{\widehat{AB}}\mathit{\mathbf{F}}(x,y)\cdot \mathrm{d}\mathit{\mathbf{r}}$$

其中 $ \mathrm{d}\mathit{\mathbf{r}}=(\mathrm{d}x,\mathrm{d}y) $ ，有向线段 $ \widehat{AB} $ 称为积分路径

由定义看出，外力 $\textit{F}$ 沿曲线 $\textit{L}$ 从 $\textit{A}$ 到 $ $\textit{B}$ $ 所做的功可表示为第二类曲线积分

$$W=\int_{\widehat{AB}}\mathit{\mathbf{F}}(x,y)\cdot \mathrm{d}\mathit{\mathbf{r}}$$
...
$$\oint_{\widehat{AB}}P(x,y)\mathrm{d}x+Q(x,y)\mathrm{d}y$$

的值只与起点 $\textit{A}$，终点 $\textit{B}$ 的坐标有关，而与连接 $\textit{A,B}$ 两点的积分路径无关。

### 命题
在区域 $\textit{D}$ 内任意选定两点 $\textit{A,B}$，曲线积分 $ \oint_{\widehat{AB}}P(x,y)\mathrm{d}x+Q(x,y)\mathrm{d}y $ 在区域 $\textit{D}$ 内与路径无关的充分必要条件是：对于 $\textit{D}$ 内任意一条简单逐段光滑的闭合曲线 $\textit{C}$，沿 $\textit{C}$ 的曲线积分为零，即
...
根据这张图，我们可以把闭合曲线 $\textit{C}$ 分为 $ \widehat{AnB} $ 和 $ \widehat{AmB} $ 

$$\oint_{\widehat{AnB}}P(x,y)\mathrm{d}x+Q(x,y)\mathrm{d}y=\oint_{\widehat{AmB}}P(x,y)\mathrm{d}x+Q(x,y)\mathrm{d}y$$

移项得

$$
\begin{align*}
0=&\oint_{\widehat{AmB}}P(x,y)\mathrm{d}x+Q(x,y)\mathrm{d}y-\oint_{\widehat{AnB}}P(x,y)\mathrm{d}x+Q(x,y)\mathrm{d}y\\
=&\oint_{\widehat{AmB}}P(x,y)\mathrm{d}x+Q(x,y)\mathrm{d}y+\oint_{\widehat{BnA}}P(x,y)\mathrm{d}x+Q(x,y)\mathrm{d}y\\
=&\oint_{C^{+}}Pdx+Qdy
\end{align*}
$$
...
设函数 $ P(x,y),Q(x,y) $ 在单连通区域 $\textit{D}$ 内有连续的一阶偏导数，对任意两点 $ A,B\in D $ ，曲线积分 $ \oint_{\widehat{AB}}P(x,y)\mathrm{d}x+Q(x,y)\mathrm{d}y $ 与路径无关的充要条件是： $ P(x,y)\mathrm{d}x+Q(x,y)\mathrm{d}y $ 恰是某个函数 $ u(x,y) $ 的全微分，此外，当 $ P(x,y)\mathrm{d}x+Q(x,y)\mathrm{d}y $ 是 $ u(x,y) $ 的全微分时，有

$$\oint_{\widehat{AB}}P(x,y)\mathrm{d}x+Q(x,y)\mathrm{d}y=\int_{A}^{B}\mathrm{d}u=u(B)-u(A)$$
...
$$
\iiint\limits_{\Omega} \left( \frac{\partial P}{\partial x} + \frac{\partial Q}{\partial y} + \frac{\partial R}{\partial z} \right) \mathrm{d}v = \iint\limits_{\Sigma} \left( P \, \mathrm{d}y \, \mathrm{d}z + Q \, \mathrm{d}z \, \mathrm{d}x + R \, \mathrm{d}x \, \mathrm{d}y \right)
$$

右侧的式子为曲面在 $ xOy,yOz,xOz $ 平面上的投影大小，用方向余弦我们可以转化为

$$
\iiint_{\Omega} \left( \frac{\partial P}{\partial x} + \frac{\partial Q}{\partial y} + \frac{\partial R}{\partial z} \right) \mathrm{d}v = \int\int_{\Sigma} \left( P \cos \alpha + Q \cos \beta + R \cos \gamma \right) \mathrm{d}S
$$
...
$$
\iiint_{\Omega} \frac{\partial R}{\partial z} \mathrm{d}v = \int\int_{\Sigma} R(x,y,z) \, \mathrm{d}x \, \mathrm{d}y
$$

同理

$$
\iiint_{\Omega} \frac{\partial P}{\partial x} \mathrm{d}v = \int\int_{\Sigma} P(x,y,z) \, \mathrm{d}y \, \mathrm{d}z
$$

$$
\iiint_{\Omega} \frac{\partial Q}{\partial y} \mathrm{d}v = \int\int_{\Sigma} Q(x,y,z) \, \mathrm{d}x \, \mathrm{d}z
$$
...
物理上我们称曲面积分 $ \iint\limits_{S^+}\mathit{\mathbf{F}}\cdot \mathbf{ \textit{n} }\mathrm{d}s $ 称为向量场 $ \mathit{\mathbf{F}}(x,y,z) $ 通过曲面 $\textit{S}$  的通量，高斯公式的物理意义其实就是 $\mathit{\mathbf{F}}$ 通过曲面 $\textit{S}$  的通量等于其散度在 $\textit{S}$  所包围的区域 $ \Omega $ 上的三重积分。需要注意的是，如果是一个闭合曲面，则外表面的积分是正，内表面的是负，如果是不闭合的曲面，我们称法向量朝上的是正，法向量朝下的是负，这个符号是要单独加在积分符号前面的和后面的积分的量并没有关系。
...
我们记 $ \mathit{\mathbf{F}}=(P,Q,R) $ 称为向量场 $\mathit{\mathbf{F}}$ 的旋度，并定义

$$
rot\mathit{\mathbf{F}}=\{\frac{\partial R}{\partial y}-\frac{\partial Q}{\partial z},\frac{\partial P}{\partial z}-\frac{\partial R}{\partial x},\frac{\partial Q}{\partial x}-\frac{\partial P}{\partial y}\}
$$

那么斯托克斯公式我们可以写成

$$
\oint_{L^+}P\mathrm{d}x+Q\mathrm{d}y+R\mathrm{d}z=\iint\limits_{S^+}rot\mathit{\mathbf{F}}\cdot \mathrm{d}\mathbf{ \textit{S} }
$$
...
rot\mathit{\mathbf{F}}=
\begin{vmatrix}
\mathit{\mathbf{i}}&\mathit{\mathbf{j}}&\mathit{\mathbf{k}}\\
\dfrac{\partial}{\partial x}&\dfrac{\partial}{\partial y}&\dfrac{\partial}{\partial z}\\
P&Q&R\\
\end{vmatrix}
\end{align*}
...
已知 $ S^+ $ 指定一侧的法向量的方向余弦为 $ (\cos\alpha,\cos\beta,\cos\gamma) $ ，那么 $ \mathrm{d}\mathit{\mathbf{S}}=(\cos\alpha,\cos\beta,\cos\gamma)\mathrm{d}S $ 
我们可以把带行列式的斯托克斯公式改写为
$$
\begin{align}
\oint_{L^+}P&\mathrm{d}x+Q\mathrm{d}y+R\mathrm{d}z=\iint\limits_{S^+}
...
$$
\mathit{\mathbf{u}}=\mathit{\mathbf{F}}(M)\equiv P(M)\mathit{\mathbf{i}}+Q(M)\mathit{\mathbf{j}}+R(M)\mathit{\mathbf{k}}
$$
...
$$
\mathrm{grad}f\mid_{\substack{(x_0,y_0,z_0)}}=\{\frac{\partial f}{\partial x},\frac{\partial f}{\partial y},\frac{\partial f}{\partial z}\}\mid_{\substack{(x_0,y_0,z_0)}}
$$
...
$$\nabla u=\frac{\partial u}{\partial x}\mathit{\mathbf{i}}+\frac{\partial u}{\partial y}\mathit{\mathbf{j}}+\frac{\partial u}{\partial z}\mathit{\mathbf{k}}\qquad\mbox{或}\qquad\nabla=\frac{\partial}{\partial x}\mathit{\mathbf{i}}+\frac{\partial}{\partial y}\mathit{\mathbf{j}}+\frac{\partial}{\partial z}\mathit{\mathbf{k}}$$


在物理学中，我们定义了电场线和磁场线，我们在数学中定义，如果在向量场 $\mathit{\mathbf{F}}$ 中，若曲线 $ \Gamma $ 上每一点 $\textit{M}$ 处的切线都与向量场 $\mathit{\mathbf{F}}$ 在 $\textit{M}$ 点的向量 $ \mathit{\mathbf{F}}\mid_{\substack{M}} $ 共线，则 $ \Gamma $ 被称为向量线。

对于数量场 $\textit{u}$ 来说，它的梯度 $ \mathrm{grad}u $ 所形成的向量场的向量线与它穿越的 $\textit{u}$ 的等值面垂直。

## 通量和散度
设 $ \mathbf{ \textit{u} }=\mathit{\mathbf{F}}(M) $ 是给定的一个向量场，假设 $\textit{S}$ 是一个双侧曲面，并取定一侧的法向量为 $\mathbf{ \textit{n} }$，则 $ \mathit{\mathbf{F}}(M) $ 在 $\textit{S}$ 上按指定一侧的第二类曲面积分为

$$
\iint\limits_{S}\mathit{\mathbf{F}}\cdot \mathbf{ \textit{n} } \mathrm{d}S
$$

的值称为向量场 $ \mathit{\mathbf{F}} $ 通过曲面 $\textit{S}$ 在指定一侧的通量

 $ \mathit{\mathbf{F}} $ 是流量场时，积分的值恰好时单位时间内流过 $\textit{S}$ 的流量的代数和，如果 $\textit{S}$ 是一个闭曲面，法向量取外法向量时，通量就是曲面上整体的流出量和流入量之差，当通量大于0时，流出的量多于流入的量。

介绍了通量之后，我们介绍散度的概念。

设 $ M_{0}(x_{0},y_{0},z_{0}) $ 是向量场 $ \mathit{\mathbf{F}}=(P,Q,R) $ 中的一点，$\textit{V}$ 是包含 $ M_{0} $ 的一个区域，其边界 $\textit{S}$ 是光滑曲面，在 $\textit{S}$ 上取定单位外法向量 $\mathit{\mathbf{n}}$，我们由高斯定理可得

$$
\iint_{S} \mathbf{F} \cdot \mathbf{n} \, \mathrm{d}S =
\iiint_{V} \left( \frac{\partial P}{\partial x} + \frac{\partial Q}{\partial y} + \frac{\partial R}{\partial z} \right) \, \mathrm{d}v
$$

或写成

$$
\iint_{S} \mathbf{F} \cdot \mathbf{n} \, \mathrm{d}S = \iiint_{V} \mathrm{div} \, \mathbf{F} \, \mathrm{d}v
$$
由积分中值定理我们可以得到

$$

\iiint\limits_{V} \mathrm{div} \, \mathit{\mathbf{F}} \, \mathrm{d}v = \mathrm{div} \, \mathit{\mathbf{F}} \bigg|_{\substack{(\bar{x},\bar{y},\bar{z})}} \cdot m(V)
$$

其中 $ m(V) $ 表示的 $\textit{V}$ 的体积，这样当积分区域V缩小到一点 $ M_{0}(x_{0},y_{0},z_{0}) $ 时，通量的平均值的极限即是 $ \mathrm{d}{div}\mathit{\mathbf{F}} $ 在 $ M_{0} $ 的值

$$
\mathrm{div} \, \mathbf{F} \mid_{(M_{0})} = \lim_{V \rightarrow M_{0}} \frac{\iint_{S} \mathbf{F} \cdot \mathbf{n} \, \mathrm{d}S}{m(V)}
$$
...
若向量场 $\mathit{\mathbf{F}}$ 使得散度 $ \mathrm{div}(M)\equiv0 $ 则成向量场 $\mathit{\mathbf{F}}$ 为无源场，也称为管形场。

若 $\mathit{\mathbf{F}}$ 为无源场，则通过该场中任何一个闭曲面沿外法向量的通量为零，在一个无源无漏的场中流入一个闭曲面的量等于流出这个闭曲面的量，我们可以想象一个宽度不均匀的水管，单位时间在不同大小截面流过的水的体积是相等的，由于截面积 $\textit{S}$ 的大小不一样，差异主要是流速大小，但是我们研究不同截面的通量是相等的。

$$\nabla=\frac{\partial}{\partial x}\mathit{\mathbf{i}}+\frac{\partial}{\partial y}\mathit{\mathbf{j}}+\frac{\partial}{\partial z}\mathit{\mathbf{k}}$$

我们将向量场 $ \mathit{\mathbf{F}}=P\mathit{\mathbf{i}}+Q\mathit{\mathbf{j}}+R\mathit{\mathbf{k}} $ 的散度

$$\mathrm{div}\mathit{\mathbf{F}}=\dfrac{\partial P}{\partial x}+\dfrac{\partial Q}{\partial y}+\dfrac{\partial R}{\partial z}$$

我们写成

$$\mathrm{div}\mathit{\mathbf{F}}=\nabla \cdot \mathit{\mathbf{F}}$$

## 环量和旋度

假设 $ \mathit{\mathbf{F}}=P\mathit{\mathbf{i}}+Q\mathit{\mathbf{j}}+R\mathit{\mathbf{k}} $ 是一个向量场，而L是向量场中给定的一条有定向的闭曲线，我们称曲线积分
...
$$\oint_{L}\mathit{\mathbf{F}}\cdot \mathrm{d}\mathit{\mathbf{r}}$$

其中 $ \mathrm{d}\mathit{\mathbf{r}}=(\mathrm{d}x,\mathrm{d}y,\mathrm{d}z) $ 

当 $\mathit{\mathbf{F}}$ 是一个静力场时，其环量 $\textit{I}$ 代表 $\mathit{\mathbf{F}}$ 沿曲线作用一周时所做的功，当 $\mathit{\mathbf{F}}$ 是一个流速场时， $ \mathit{\mathbf{F}}\cdot \mathrm{d}\mathit{\mathbf{r}} $ 则是曲线 $\textit{L}$ 上一点处的流速在 $\textit{L}$ 切线方向的投影乘以相应的弧微分。因此在流速场中沿一条有向闭曲线之环量是流速沿曲线切线方向投影的某种代数和，它的物理意义在于标明这个闭曲线整体是否旋转。当沿着曲线 $\textit{L}$ 流速场有旋转时，则沿 $\textit{L}$ 的环量不等于0.

为了研究向量场每一点的旋转情况，我们需要引进环量密度这个概念。在向量场 $\mathit{\mathbf{F}}$ 中任意一点 $\textit{M}$ ，在 $\textit{M}$ 处任意取定一个单位向量 $\mathit{\mathbf{n}}$ ，过点 $\textit{M}$ 任作一小曲面 $\textit{S}$ ，并设 $\textit{L}$ 为曲面 $\textit{S}$ 的正向边界，与 $ \textit{n} $ 组成右手系，当曲面 $\textit{S}$ 收缩成一点 $\textit{M}$ 时，即当S的直径 $ \lambda(S)\rightarrow 0 $ 时，若极限

$$
\dfrac{\lim\limits_{\lambda(S)\rightarrow 0}\oint_{L}\mathit{\mathbf{F}}\cdot \mathrm{d}\mathit{\mathbf{r}}}{m(S)}
$$

存在，则该极限值为向量场 $\mathit{\mathbf{F}}$ 在 $\textit{M}$ 点绕方向$\mathit{\mathbf{n}}$ 的环量面密度，或称为方向旋量。
...
为向量场 $\mathit{\mathbf{F}}$ 在一点M处的旋度，记为 $ \mathrm{rot}\mathit{\mathbf{F}} $ ，那么 $\mathit{\mathbf{F}}$ 在点M处沿任一方向n的方向旋量为

$$
\mathrm{rot}\mathit{\mathbf{F}}\cdot \mathbf{ \textit{n} }=\mid \mathrm{rot}\mathit{\mathbf{F}}\mid \cos<\mathbf{ \textit{n} },\mathrm{rot}\mathit{\mathbf{F}}>
$$

也就是说沿n的方向旋量恰好等于旋度 $ \mathrm{rot}\mathit{\mathbf{F}} $ 在n上的投影，方向旋量沿旋度的方向达到最大值，也就是旋度达到方向是方向旋量最大的方向。
...
设 $ \mathit{\mathbf{F}}=P\mathit{\mathbf{i}}+Q\mathit{\mathbf{j}}+R\mathit{\mathbf{k}} $ 是定义在区域 $ D\subset R^3 $ 中的一个向量场，若沿 $\textit{D}$ 中的任意一条曲线 $ \widehat{AB} $ 的积分

$$
\int_{\widehat{AB}}\mathit{\mathbf{F}}\cdot \mathrm{d}\mathit{\mathbf{r}}=\int_{\widehat{AB}}P\mathrm{d}x+Q\mathrm{d}y+R\mathrm{d}z
$$

只与曲线的起点 $\textit{A}$ 和终点 $\textit{B}$ 有关，而与曲线 $ \widehat{AB} $ 的路径无关，则称向量场 $\mathit{\mathbf{F}}$ 在 $\textit{D}$ 内是一个保守场。

设D是一个线性单连通区域，而 $ \mathit{\mathbf{F}}=P\mathit{\mathbf{i}}+Q\mathit{\mathbf{j}}+R\mathit{\mathbf{k}} $ 是定义在区域 $ D $ 中的一个向量场，其中P,Q,R在D内有一阶连续偏导数，那么下列三个条件是等价的


- $ \mathit{\mathbf{F}} $  在D内是保守场
- $ \mathit{\mathbf{F}} $  在D内沿任意闭曲线 $ \Gamma $ 的环量为零，即 $ \int_{\widehat{AB}}\mathit{\mathbf{F}}\cdot \mathrm{d}\mathit{\mathbf{r}}=0 $ 
- $ \mathit{\mathbf{F}} $  是无旋场，即 $ \mathrm{rot}\mathit{\mathbf{F}}=0 $ 


与保守场相关的是有势场，如果对于向量场 $\mathit{\mathbf{F}}$ ，存在一个函数 $\textit{f}$ 使得其负梯度恰好是 $\mathit{\mathbf{F}}$，也即

$$
\mathit{\mathbf{F}}=-\mathrm{grad}f
$$
