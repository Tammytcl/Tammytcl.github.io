...
令 $ \lambda = \max\limits_{1\leq i \leq n}{\Delta s_{i}} $ ，我们定义变力\mathit{\mathbf{F}}沿 $ \overarc{AB} $ 所做的功为
...
设 $ \mathit{\mathbf{F}}=P\mathit{\mathbf{i}}+Q\mathit{\mathbf{j}}+R\mathit{\mathbf{k}} $ 是定义在区域 $ D\subset R^3 $ 中的一个向量场，若沿 $\textit{D}$ 中的任意一条曲线 $ \overarc{AB} $ 的积分

$$
\int_{\overarc{AB}}\mathit{\mathbf{F}}\cdot \mathrm{d}\mathit{\mathbf{r}}=\int_{\overarc{AB}}P\mathrm{d}x+Q\mathrm{d}y+R\mathrm{d}z
$$

只与曲线的起点\textit{A}和终点 $\textit{B}$ 有关，而与曲线 $ \overarc{AB} $ 的路径无关，则称向量场\mathit{\mathbf{F}}在 $\textit{D}$ 内是一个保守场。
...
- $ \mathit{\mathbf{F}} $  在D内沿任意闭曲线 $ \Gamma $ 的环量为零，即 $ \int_{\overarc{AB}}\mathit{\mathbf{F}}\cdot \mathrm{d}\mathit{\mathbf{r}}=0 $ \\
