...
同理全体实数组 $ (x,y,z) $  所组成的集合记作 $ \mathit{\mathbf{r}}^{3} $  即 $ \mathit{\mathbf{r}}^{3}= \left\{(x,y,z)\mid x\in \mathit{\mathbf{r}}, y\in\mathit{\mathbf{r}},z\in\mathbf{R}\right\} $ 

对于更一般的  $ \textit{n} $  元函数，我们构建  $ \textit{n} $  个有次序的实数组： $ (x_{1},x_{2},\cdots,x_{n}) $ ，构成 $ \mathit{\mathbf{r} $  }^{n} $ 
即
$$ \mathit{\mathbf{r}}^{n}= \left\{(x_{1},x_{2},\cdots,x_{n})|x_{j}\in\mathit{\mathbf{r}},j=1,2,\dots,n\right\} $ $
