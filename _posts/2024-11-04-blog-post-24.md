...
多元函数顾名思义就是多个变量的函数，我们生活常见的应用相关的问题也大多是多元函数的问题。我们在介绍一元函数的时候，我们一开始是通过映射去定义函数的，二元函数是两个变量在一定范围内变化时，相应的数组对应平面上某一点集合。那么同理我们可以知道三元函数的实质就是三维空间中某个点集合到实数 $ \textit{\mathbf{r}}z4的一个映射。

我们用数学语言来给出定义：
我们将全体实数组 $ (x,y) $  所组成的集合记作 $ \textit{\mathbf{r}}^{2} $  即

$$\mathbf{textit{r}}^{2}= \left\{(x,y)|x\in \textit{\mathbf{r}}, y\in \mathbf{R}\right\}$$

同理全体实数组 $ (x,y,z) $  所组成的集合记作 $ \textit{\mathbf{r}}^{3} $  即 $ \textit{\mathbf{r}}^{3}= \left\{(x,y,z)|x\in \textit{\mathbf{r}}, y\in\textit{\mathbf{r}},z\in\mathbf{R}\right\} $ 

对于更一般的  $ \textit{n} $  元函数，我们构建  $ \textit{n} $  个有次序的实数组： $ (x_{1},x_{2},\cdots,x_{n}) $ ，构成 $ \textit{\mathbf{r} $  }^{n} $ 
即 $ \textit{\mathbf{r}}^{n}= \left\{(x_{1},x_{2},\cdots,x_{n})|x_{j}\in\textit{\mathbf{r}},j=1,2,\dots,n\right\} $ 

下面我们给出多元函数的正式定义：
### 定义
设有一个集合 $ D\subset \textit{\mathbf{r} $  }^{n} $ ，如果对于集合 $  $\textit{D}$ $  中每一点 $ (x_{1},x_{2},\cdots,x_{n}) $ ，按照一定的规则 $ \textit{f} $  ，都有一个唯一确定的实数 $ u\in \textit{\mathbf{r}} $  与之相对应，则称 $ \textit{f} $  是一个定义在  $\textit{D}$ 上的 $ \textit{n} $ 元函数，这里的 $\textit{D}$ 称为 $ \textit{f} $ 的定义域，与 $ (x_{1},x_{2},\cdots,x_{n}) $ 相对应的数 $ \textit{u} $ 被称为 $ \textit{f} $  在点 $ (x_{1},x_{2},\cdots,x_{n}) $  的值，并记为 $ f(x_{1},x_{2},\cdots,x_{n}) $ ，全体函数值的集合。

$$f(D)=\left\{f(x_{1},x_{2},\cdots,x_{n})|(x_{1},x_{2},\cdots,x_{n})\in D\right\}$$

我们在此谈论的 $ \textit{n} $ 元函数其实是 $ \textit{\mathbf{r}}^{n} $ 到 $ \textit{\mathbf{r}} $  的一个映射，我们接下来再将其一般化为 $ \textit{\mathbf{r}}^{n} $  到 $ \textit{\mathbf{r}}^{m} $  的映射，最常见的就是极坐标中的代数方程了。

设  $\textit{D}$    为 $ \textit{\mathbf{r}}^{n} $  的一个集合，又设 $ \textit{f} $  是 $ D\rightarrow\textit{\mathbf{r}}^{m} $  的一个映射，那么对于 $  $\textit{D}$ $  中的每一个点 $ (x_{1},x_{2},\cdots,x_{n}) $  ，在 $ \textit{\mathbf{r}}^{m} $  中有唯一确定的点 $ (y_{1},y_{2},\cdots,y_{m}) $  与之相对应，这里每一个
 $ y_{j}(j=1,2,\dots m) $  都是由 $ (x_{1},x_{2},\cdots,x_{n}) $  所确定的。

故 $ y_{j} $  是 $ (x_{1},x_{2},\cdots,x_{n}) $  的一个函数，设它为 $ f_{j}(x_{1},x_{2},\cdots,x_{n})(j=1,2,\dots m) $  这样映射 $ f:D\rightarrow\textit{\mathbf{r}}^{m} $  相当于 $\textit{M}$ 个  $ \textit{n} $  元函数
...
因此， $ \textit{\mathbf{r}}^{n} $  中的集合到 $ \textit{\mathbf{r}}^{m} $  的一个映射可以用有序的 $ \textit{m} $  个  $ \textit{n} $  元函数表示，其中第 $ \textit{j} $  个  $ \textit{n} $  元函数 $ f_{j} $  称作映射的第 $ \textit{j} $  个分量。
...
在数轴 $ \textit{\mathbf{r}} $  上点 $ x $  到 $ x_{0} $  的距离为：
 $ \mid x-x_{0}\mid  = \sqrt{(x-x_{0})^{2}} $ 

在平面 $ \textit{\mathbf{r}}^{2} $  中点 $ P(x,y) $  到点 $ P(x_{0},y_{0}) $  的距离为
 $ \sqrt{(x-x_{0})^{2} + (y-y_{0})^{2}} $ 

在三维空间 $ \textit{\mathbf{r}}^{3} $  中点 $ P(x,y,z) $  到点 $ P(x_{0},y_{0},z_{0}) $  的距离为 $ \sqrt{(x-x_{0})^{2} + (y-y_{0})^{2}+ (z-z_{0})^{2}} $ 

很自然的我们将 $ \textit{\mathbf{r}}^{n} $  中的点 $ P(x_{1},x_{2},\cdots,x_{n}) $  到 $ P_{0}(x_{0},x_{0},\cdots,x_{0}) $  的距离为 $ \sqrt{\sum\limits_{j=1}^{n} (x_{j}-x_{0})^{2}} $ 

### 总结
若将 $ \textit{\mathbf{r}}^{n} $  中的两点 $ \textit{P,Q} $  之间的距离记作 $ d(P,Q) $ ，那么它满足下列条件：

- $ d(P,Q)\ge 0 $ 当且仅当 $ P=Q $ 时等号成立
- $ d(P,Q) =d(Q,P)\quad \forall P,Q\in \textit{\mathbf{r}}^{n} $ 
- $ d(P,Q)\le d(P,R) + d(R,Q)\quad\forall P,Q,R\in\textit{\mathbf{r}}^{n} $ 


### 定义
设 $ P_{0}\in\textit{\mathbf{r}}^{n} $  为给定的一点， $ \textit{r} $ 是给定的正数，我们定义 $ P_{0} $  点的  $ \textit{r} $ 邻域是集合

$$U_{r}(P_{0}) = \left\{P\in \textit{\mathbf{r}}^{n}|d(P_{0},P)<r\right\}$$

当 $ n=1 $  时，一点 $ P_{0}\in\textit{\mathbf{r}} $  的  $ \textit{r} $ 邻域是以该点为中心、 $ \textit{r} $ 为半径的开区间。当 $ n=2 $  时，一点 $ P_{0}\in\textit{\mathbf{r}}^{2} $  的  $ \textit{r} $ 邻域是以该点为中心、 $ \textit{r} $ 为半径的圆的内部（不包含边界点），通常称之为开圆。当 $ n=3 $  时，一点 $ P_{0}\in\textit{\mathbf{r}}^{3} $  的  $ \textit{r} $ 邻域是以该点为中心、 $ \textit{r} $ 为半径的球的内部（不包含球面），通常称之为开球。

在一元函数的微积分中，我们遇到的函数大多定义在一个区间上，它是数轴上的一段，但在多元函数中函数的定义域将会更加复杂，我们将介绍区域的概念来代替过去的开区间。

设 $ E\subset \textit{\mathbf{r}}^{n} $  是一个给定的集合，根据集合 $  $ \textit{E} $ $  我们将
 $ \textit{\mathbf{r}}^{n} $  中的点分为三类： $  $ \textit{E} $ $ 的内点、外点和边界点。

一点 $ P\in\textit{\mathbf{r}}^{n} $  称为 $  $ \textit{E} $ $  的内点，如果存在一个正数 $ \textit{r} $  使得 $ P $ 点的\ $ \textit{r} $  邻域整个包含于 $ \textit{E} $ 
$$U_{r}(P)\subset E$$

一点 $ P\in\textit{\mathbf{r}}^{n} $ 称为 $ \textit{E} $ 的外点，如果存在一个正数r使得 $ P $ 点的\ $ \textit{r} $  邻域与 $ \textit{E} $ 不相交
$$U_{r}(P)\cap E = \varnothing $$

既非内点又非外点的点称为 $ \textit{E} $ 的边界点一点 $ P\in\textit{\mathbf{r}}^{n} $ 称为 $ \textit{E} $ 的边界点，如果存在一个正数r使得 $ P $ 点的\ $ \textit{r} $  邻域中既有 $ \textit{E} $ 的点又有非 $ \textit{E} $ 的点。

我们用 $ \partial E $ 表示集合 $ \textit{E} $ 中全体边界点的集合。

一个集合 $ \textit{E} $ 称为开集，如果它的每一点都是内点。如平面不带边的任意一个矩形内部，不带边的任意一个圆的内部都是 $ \textit{\mathbf{r}}^{2} $ 中的开集。 $ \textit{E} $ 是开集的充要条件是 $ \textit{E} $ 中没有边界点。

一个集合 $ \textit{E} $ 称为闭集，如果它包含着它的全部边界点。

开集和闭集是两种比较极端的情况，一般情况下的集合介于这两者之间：包含边界点但是不包含全部。

设 $ \textit{E} $ 是 $ \textit{\mathbf{r}}^{n} $ 的一个开集，如果 $ \textit{E} $ 中任意两点都可以用一条落在 $ \textit{E} $ 中的曲线相连接，则我们说 $ \textit{E} $ 是连通的。

 $ \textit{\mathbf{r}}^{n} $ 中的连同的非空开集称为 $ \textit{\mathbf{r}}^{n} $ 的区域，区域是开区间的推广。

如果存在一个正数 $ \rho $ ，使得 $ \textit{E} $ 包含于以原点为心，以 $ \rho $ 为半径的球内，则我们称集合 $ E\subset\textit{\mathbf{r}}^{n} $ 称为有界集合。如果找不到这样的 $ \rho $ ，则称为无界集合。
...
### 有界性定义
设函数 $ f(P) $ 在有界闭区间 $ \bar{D} $ 上连续，则 $ f(P) $ 在 $ \bar{D} $ 上有界，即存在常数 $ \textit{\mathbf{M}}>0 $ ，使

$$\mid f(P)\mid \leq \textit{\mathbf{M}}\quad \forall P\in\bar{D}$$
...
### 介值定理
设函数 $ f(P) $ 在有界闭区间 $ \bar{D} $ 上连续，并假定\textit{\mathbf{M}}与\textit{\mathbf{m}}分别是 $ f(P) $ 在 $ \bar{D} $ 的最大值和最小值，则对于任意的 $ \eta:\quad \textit{\mathbf{m}}\leq \eta\leq\textit{\mathbf{M}} $ ，一定有一点
...
如果函数 $ z=f(x,y) $ 在区域 $\textit{\mathbf{D}}$ 内两个二阶偏导数 $ \dfrac{\partial^{2} z}{\partial x\partial y } $ 及 $ \dfrac{\partial^{2} z}{\partial y\partial x} $ 连续，那么在该区域内，这两个二阶混合偏导数必相等。
...
其中 $ \cos \phi $ 和 $ \cos \psi $  是方向 $\textit{\mathbf{l}}$ 的方向余弦

偏导数反映的是函数沿坐标轴方向的变化率，而方向导数反映的是函数在 $ P_0 $ 点沿任意方向的变化率。

## 梯度
梯度这个概念我觉得很酷，因为它有一个算子（哈哈哈） $ \bigtriangledown=\dfrac{\partial}{\partial x}\textit{\mathbf{i}}+ \dfrac{\partial}{\partial y}\textit{\mathbf{j}} $ （更高维度也是类似就不列举了）
...
$$\mathbf{grad}f(x_{0},y_{0})=\bigtriangledown f(x_{0},y_{0})=f_{x}(x_{0},y_{0})\textit{\mathbf{i}}+f_{y}(x_{0},y_{0})\textit{\mathbf{j}}$$
...
$$\textit{\mathbf{F}}(x,y) = (\textit{\mathbf{P}}(x,y),\textit{\mathbf{Q}}(x,y))$$
...
设第i段的弧长为 $ \Delta s_{i} $ ，当 $ \Delta s_{i} $ 很小时，\textit{\mathbf{F}}在 $ \wideparen{A_{i-1}A_{i}} $ 上的变化不大，可近似的看作为常力 $ \textit{\mathbf{F}}(\xi_{i},\eta_{i}) $ ，其中 $ (\xi_{i},\eta_{i}) $ 为弧段 $ \wideparen{A_{i-1}A_{i}} $ 上任取的一点，同时可将指点的运动轨迹 $ \wideparen{A_{i-1}A_{i}} $ 近似的看作从 $ A_{i-1} $ 到 $ A_{i} $ 的一条直线，于是\textit{\mathbf{F}}在这段上做的功 $ \Delta W_{i} $ ，近似为

$$\Delta W_{i}\approx \textit{\mathbf{F}}(\xi_{i},\eta_{i}) \cdot \overrightarrow{A_{i-1}A_{i}}$$

设 $ \textit{\mathbf{F}}(x,y)=P(x,y)\textit{\mathbf{i}}+Q(x,y)\textit{\mathbf{j}} $ 

$$\overrightarrow{A_{i-1}A_{i}}=\Delta \textit{\mathbf{i}}+\Delta y_{i}\textit{\mathbf{j}}$$
...
令 $ \lambda = \max\limits_{1\leq i \leq n}{\Delta s_{i}} $ ，我们定义变力\textit{\mathbf{F}}沿 $ \wideparen{AB} $ 所做的功为
...
### 定义} 
设 $\textit{L}$ 是从 $\textit{A}$ 到 $ $\textit{B}$ $ 的一条有向分段光滑曲线，向量函数 $ \textit{\mathbf{F}}(x,y) = \textit{\mathbf{P}}(x,y)\textit{\mathbf{i}}+\textit{\mathbf{Q}}(x,y)\textit{\mathbf{j}} $ 在 $\textit{L}$ 上有定义，按 $\textit{L}$ 的方向顺序用分点 $ A=A_{0}(x_{0},y_{0}),A_{1}(x_{1},y_{1}),\cdots,A_{n}(x_{n},y_{n})=B $ 将 $\textit{L}$ 分成 $ \textit{n} $ 个有向线段 $ \wideparen{A_{i-1}A_{i}}\quad(i=1,2,\cdots,n) $ ， $ \wideparen{A_{i-1}A_{i}} $ 的弧长记为 $ \Delta s_{i} $ ，并令 $ \lambda = \max\limits_{1\leq i \leq n}{\Delta s_{i}} $ ，在 $ \wideparen{A_{i-1}A_{i}} $ 上任取一点 $ (\xi_{i},\eta_{i}) $ ，若极限

$$\lim\limits_{\lambda\rightarrow 0}\sum_{i=1}^{n}\textit{\mathbf{F}}(\xi_{i},\eta_{i}) \cdot \overrightarrow{A_{i-1}A_{i}}=\lim\limits_{\lambda\rightarrow 0}\sum_{i=1}^{n}[P(\xi_{i},\eta_{i})\Delta x_{i}+Q(\xi_{i},\eta_{i})\Delta y_{i}]$$

存在，其中 $ \Delta x_{i}=x_{i}-x_{i-1},\Delta y_{i}=y_{i}-y_{i-1} $ ，不依赖曲线 $\textit{L}$ 的任意分割法及中间点 $ (\xi_{i},\eta_{i},\zeta_{i}) $ 的取法，则称此极限为向量值函数 $ \textit{\mathbf{F}}(x,y) $ 沿曲线 $\textit{L}$ 从 $\textit{A}$ 到 $\textit{B}$ 的第二类曲线积分，也叫做对坐标的曲线积分，记作

$$\int_{\wideparen{AB}}P\mathrm{d}x+Q\mathrm{d}y \quad or \quad  \int_{\wideparen{AB}}\textit{\mathbf{F}}(x,y)\cdot \mathrm{d}\textit{\mathbf{r}}$$

其中 $ \mathrm{d}\textit{\mathbf{r}}=(\mathrm{d}x,\mathrm{d}y) $ ，有向线段 $ \wideparen{AB} $ 称为积分路径

由定义看出，外力 $\textit{F}$ 沿曲线 $\textit{L}$ 从 $\textit{A}$ 到 $ $\textit{B}$ $ 所做的功可表示为第二类曲线积分

$$W=\int_{\wideparen{AB}}\textit{\mathbf{F}}(x,y)\cdot \mathrm{d}\textit{\mathbf{r}}$$

第二类曲线积分是要考虑正负的，设 $ L $ 是有向光滑曲线， $ L^- $ 是 $ L $ 的反向曲线弧

$$
\int_{L^-}\textit{\mathbf{F}}(x,y)\cdot \mathrm{d}\textit{\mathbf{r}}=-\int_{L}\textit{\mathbf{F}}(x,y)\cdot \mathrm{d}\textit{\mathbf{r}}
$$
...
我们说 $\textit{L}$ 是一条简单曲线，如果它是一个映射 $ \phi:[\alpha,\beta]\rightarrow \textit{\mathbf{r}}^{2} $ 的像，其中 $ \phi $ 在 $ [\alpha,\beta] $ 上连续， $ \phi $ 在 $ (\alpha,\beta) $ 上是一一映射，且 $ \phi(\alpha)=\phi(\beta) $ 
...
### 定义
设 $\textit{S}$是一个分片光滑的双侧曲面，在S上选定了一侧，记选定侧的法向量为 $ \mathbf{ $ \textit{n} $ }(P) $ ，假设在 $\textit{S}$  上给定了一个向量函数 $ \textit{\mathbf{F}}(x,y,z) $ ，我们将 $\textit{S}$  分割成 $ \textit{n} $ 个不相重叠的小曲面片 $ \Delta S_{i} $ ，面积也记为 $ \Delta S_{i} $ ，在 $ \Delta S_{i} $ 上任取一点 $ M_{i}(\xi_{i},\eta_{i},\zeta_{i}) $ 

$$\sum_{i=1}^{n}\textit{\mathbf{F}}(\xi_{i},\eta_{i},\zeta_{i})\cdot \mathbf{ $ \textit{n} $ }\Delta S_{i}$$

令 $ \lambda $ 是 $ \Delta S_{i} $ 中直径的最大者，如果上述求和对 $\textit{S}$  的任意一种分割及中间点 $ (\xi_{i},\eta_{i},\zeta_{i}) $ 的任意的选取，当 $ \lambda\rightarrow 0 $ 时总有极限，那么称此极限为向量函数 $ \textit{\mathbf{F}}(x,y,z) $ 在 $\textit{S}$  上的第二类曲面积分，或写成

$$\iint\limits_{S}\textit{\mathbf{F}}\cdot \mathrm{d}\mathbf{ \textit{S}}\qquad\mbox{其中}\mathrm{d}\textit{\mathbf{S}}=\mathbf{ $ \textit{n} $ }(x,y,z)\mathrm{d}S$$
...
$$\textit{\mathbf{F}}=(P(x,y,z),Q(x,y,z),R(x,y,z))$$的散度，记作 $ div\textit{\mathbf{F}} $ 

公式可以写为

$$\iint\limits_{S^+}\textit{\mathbf{F}}\cdot \mathbf{ $ \textit{n} $ }\mathrm{d}s=\iiint\limits_{\Omega}div\textit{\mathbf{F}}\mathrm{d}V$$

物理上我们称曲面积分 $ \iint\limits_{S^+}\textit{\mathbf{F}}\cdot \mathbf{ $ \textit{n} $ }\mathrm{d}s $ 称为向量场 $ \textit{\mathbf{F}}(x,y,z) $ 通过曲面 $\textit{S}$  的通量，高斯公式的物理意义其实就是\textit{\mathbf{F}}通过曲面 $\textit{S}$  的通量等于其散度在 $\textit{S}$  所包围的区域 $ \Omega $ 上的三重积分。需要注意的是，如果是一个闭合曲面，则外表面的积分是正，内表面的是负，如果是不闭合的曲面，我们称法向量朝上的是正，法向量朝下的是负，这个符号是要单独加在积分符号前面的和后面的积分的量并没有关系。
...
设 $\textit{S}$  为分片光滑的双侧曲面，其边线 $\textit{L}$ 是一条或几条分段光滑的闭曲线，假定在 $\textit{S}$  上取定一侧的单位法向量\mathbf{ $ \textit{n} $ }，规定 $\textit{L}$ 的定向，使得 $\textit{L}$ 的定向与\mathbf{ $ \textit{n} $ }的指向构成右手系。记 $ S^+ $ 及 $ L^+ $ 分别为给定定向后的 $\textit{S}$  与 $\textit{L}$ ，若 $ P(x,y,z),Q(x,y,z),R(x,y,z) $ 是 $ S+L $ 上的有一阶连续偏导数的函数，则有斯托克斯公式：
...
&= \iint\limits_{D} \left( \frac{\partial P}{\partial z}(-f_{y}) - \frac{\partial P}{\partial y} \right) \mathrm{d}x\,\mathrm{d}y \\
&= -\iint\limits_{D} \left( \frac{\partial P}{\partial y} + \frac{\partial P}{\partial z}f_{y} \right) \mathrm{d}x\,\mathrm{d}y
\end{align*}
$$
...
我们记 $ \textit{\mathbf{F}}=(P,Q,R) $ 称为向量场 $\textit{\mathbf{F}}$ 的旋度，并定义

$$
rot\textit{\mathbf{F}}=\{\frac{\partial R}{\partial y}-\frac{\partial Q}{\partial z},\frac{\partial P}{\partial z}-\frac{\partial R}{\partial x},\frac{\partial Q}{\partial x}-\frac{\partial P}{\partial y}\}
$$

那么斯托克斯公式我们可以写成

$$
\oint_{L^+}P\mathrm{d}x+Q\mathrm{d}y+R\mathrm{d}z=\iint\limits_{S^+}rot\textit{\mathbf{F}}\cdot \mathrm{d}\mathbf{ \textit{S} }
$$
...
rot\textit{\mathbf{F}}=
\begin{vmatrix}
\textit{\mathbf{i}}&\textit{\mathbf{j}}&\textit{\mathbf{k}}\\
\dfrac{\partial}{\partial x}&\dfrac{\partial}{\partial y}&\dfrac{\partial}{\partial z}\\
P&Q&R\\
\end{vmatrix}
\end{align*}
...
已知 $ S^+ $ 指定一侧的法向量的方向余弦为 $ (\cos\alpha,\cos\beta,\cos\gamma) $ ，那么 $ \mathrm{d}\textit{\mathbf{S}}=(\cos\alpha,\cos\beta,\cos\gamma)\mathrm{d}S $ 
我们可以把带行列式的斯托克斯公式改写为
$$
\begin{align}
\oint_{L^+}P&\mathrm{d}x+Q\mathrm{d}y+R\mathrm{d}z=\iint\limits_{S^+}
...
$$
\textit{\mathbf{u}}=\textit{\mathbf{F}}(M)\equiv P(M)\textit{\mathbf{i}}+Q(M)\textit{\mathbf{j}}+R(M)\textit{\mathbf{k}}
$$
...
$$\nabla u=\frac{\partial u}{\partial x}\textit{\mathbf{i}}+\frac{\partial u}{\partial y}\textit{\mathbf{j}}+\frac{\partial u}{\partial z}\textit{\mathbf{k}}\qquad\mbox{或}\qquad\nabla=\frac{\partial}{\partial x}\textit{\mathbf{i}}+\frac{\partial}{\partial y}\textit{\mathbf{j}}+\frac{\partial}{\partial z}\textit{\mathbf{k}}$$


在物理学中，我们定义了电场线和磁场线，我们在数学中定义，如果在向量场 $\textit{\mathbf{F}}$ 中，若曲线 $ \Gamma $ 上每一点 $\textit{M}$ 处的切线都与向量场\textit{\mathbf{F}}在 $\textit{M}$ 点的向量 $ \textit{\mathbf{F}}\bigg|_{\substack{M}} $ 共线，则 $ \Gamma $ 被称为向量线。

对于数量场 $\textit{u}$ 来说，它的梯度 $ \mathrm{grad}u $ 所形成的向量场的向量线与它穿越的 $\textit{u}$ 的等值面垂直。

## 通量和散度
设 $ \mathbf{ \textit{u} }=\textit{\mathbf{F}}(M) $ 是给定的一个向量场，假设 $\textit{S}$ 是一个双侧曲面，并取定一侧的法向量为\mathbf{ $ \textit{n} $ }，则 $ \textit{\mathbf{F}}(M) $ 在 $\textit{S}$ 上按指定一侧的第二类曲面积分为

$$
\iint\limits_{S}\textit{\mathbf{F}}\cdot \mathbf{ $ \textit{n} $ } \mathrm{d}S
$$

的值称为向量场 $ \textit{\mathbf{F}} $ 通过曲面 $\textit{S}$ 在指定一侧的通量

 $ \textit{\mathbf{F}} $ 是流量场时，积分的值恰好时单位时间内流过 $\textit{S}$ 的流量的代数和，如果 $\textit{S}$ 是一个闭曲面，法向量取外法向量时，通量就是曲面上整体的流出量和流入量之差，当通量大于0时，流出的量多于流入的量。

介绍了通量之后，我们介绍散度的概念。

设 $ M_{0}(x_{0},y_{0},z_{0}) $ 是向量场 $ \textit{\mathbf{F}}=(P,Q,R) $ 中的一点，\textit{V}是包含 $ M_{0} $ 的一个区域，其边界 $\textit{S}$ 是光滑曲面，在 $\textit{S}$ 上取定单位外法向量\textit{\mathbf{n}}，我们由高斯定理可得

$$
\oiint\limits_{S}\textit{\mathbf{F}}\cdot \textit{\mathbf{n}}\mathrm{d}S=
...
$$
\oiint\limits_{S}\textit{\mathbf{F}}\cdot \textit{\mathbf{n}}\mathrm{d}S=
\iiint\limits_{V}\mathrm{div}\textit{\mathbf{F}}\mathrm{d}v
$$

由积分中值定理我们可以得到

$$
\iiint\limits_{V}\mathrm{d}{div}\textit{\mathbf{F}}\mathrm{d}v=
\mathrm{div}\textit{\mathbf{F}}\bigg|_{\substack{(\bar{x},\bar{y},\bar{z})}}\cdot m(V)
$$

其中 $ m(V) $ 表示的\textit{V}的体积，这样当积分区域V缩小到一点 $ M_{0}(x_{0},y_{0},z_{0}) $ 时，通量的平均值的极限即是 $ \mathrm{d}{div}\textit{\mathbf{F}} $ 在 $ M_{0} $ 的值

$$
\mathrm{div}\textit{\mathbf{F}}\bigg|_{\substack{(M_{0})}}=\lim\limits_{V\rightarrow M_{0}}\dfrac{\oiint\limits_{S}\textit{\mathbf{F}}\cdot \textit{\mathbf{n}}\mathrm{d}S}{m(V)}
$$
...
若向量场\textit{\mathbf{F}}使得散度 $ \mathrm{div}(M)\equiv0 $ 则成向量场\textit{\mathbf{F}}为无源场，也称为管形场。

若\textit{\mathbf{F}}为无源场，则通过该场中任何一个闭曲面沿外法向量的通量为零，在一个无源无漏的场中流入一个闭曲面的量等于流出这个闭曲面的量，我们可以想象一个宽度不均匀的水管，单位时间在不同大小截面流过的水的体积是相等的，由于截面积 $\textit{S}$ 的大小不一样，差异主要是流速大小，但是我们研究不同截面的通量是相等的。

$$\nabla=\frac{\partial}{\partial x}\textit{\mathbf{i}}+\frac{\partial}{\partial y}\textit{\mathbf{j}}+\frac{\partial}{\partial z}\textit{\mathbf{k}}$$

我们将向量场 $ \textit{\mathbf{F}}=P\textit{\mathbf{i}}+Q\textit{\mathbf{j}}+R\textit{\mathbf{k}} $ 的散度

$$\mathrm{div}\textit{\mathbf{F}}=\dfrac{\partial P}{\partial x}+\dfrac{\partial Q}{\partial y}+\dfrac{\partial R}{\partial z}$$

我们写成

$$\mathrm{div}\textit{\mathbf{F}}=\nabla \cdot \textit{\mathbf{F}}$$

## 环量和旋度

假设 $ \textit{\mathbf{F}}=P\textit{\mathbf{i}}+Q\textit{\mathbf{j}}+R\textit{\mathbf{k}} $ 是一个向量场，而L是向量场中给定的一条有定向的闭曲线，我们称曲线积分
...
$$\oint_{L}\textit{\mathbf{F}}\cdot \mathrm{d}\textit{\mathbf{r}}$$

其中 $ \mathrm{d}\textit{\mathbf{r}}=(\mathrm{d}x,\mathrm{d}y,\mathrm{d}z) $ 
当\textit{\mathbf{F}}是一个静力场时，其环量\textit{I}代表\textit{\mathbf{F}}沿曲线作用一周时所做的功，当\textit{\mathbf{F}}是一个流速场时， $ \textit{\mathbf{F}}\cdot \mathrm{d}\textit{\mathbf{r}} $ 则是曲线 $\textit{L}$ 上一点处的流速在 $\textit{L}$ 切线方向的投影乘以相应的弧微分。因此在流速场中沿一条有向闭曲线之环量是流速沿曲线切线方向投影的某种代数和，它的物理意义在于标明这个闭曲线整体是否旋转。当沿着曲线 $\textit{L}$ 流速场有旋转时，则沿 $\textit{L}$ 的环量不等于0.

为了研究向量场每一点的旋转情况，我们需要引进环量密度这个概念。在向量场\textit{\mathbf{F}}中任意一点 $\textit{M}$ ，在 $\textit{M}$ 处任意取定一个单位向量\textit{\mathbf{n}}，过点 $\textit{M}$ 任作一小曲面 $\textit{S}$ ，并设 $\textit{L}$ 为曲面 $\textit{S}$ 的正向边界，与 $ \textit{n} $ 组成右手系，当曲面 $\textit{S}$ 收缩成一点 $\textit{M}$ 时，即当S的直径 $ \lambda(S)\rightarrow 0 $ 时，若极限

$$
\dfrac{\lim\limits_{\lambda(S)\rightarrow 0}\oint_{L}\textit{\mathbf{F}}\cdot \mathrm{d}\textit{\mathbf{r}}}{m(S)}
$$

存在，则该极限值为向量场\textit{\mathbf{F}}在 $\textit{M}$ 点绕方向\textit{ $ \textit{n} $ }的环量面密度，或称为方向旋量。
...
\dfrac{\partial}{\partial x}&\dfrac{\partial}{\partial y}&\dfrac{\partial}{\partial z}\\
P&Q&R\\
\end{vmatrix}\cdot \mathbf{ $ \textit{n} $ }
$$
...
为向量场\textit{\mathbf{F}}在一点M处的旋度，记为 $ \mathrm{rot}\textit{\mathbf{F}} $ ，那么\textit{\mathbf{F}}在点M处沿任一方向n的方向旋量为

$$
\mathrm{rot}\textit{\mathbf{F}}\cdot \mathbf{ $ \textit{n} $ }=\mid \mathrm{rot}\textit{\mathbf{F}}\mid \cos<\mathbf{ $ \textit{n} $ },\mathrm{rot}\textit{\mathbf{F}}>
$$

也就是说沿n的方向旋量恰好等于旋度 $ \mathrm{rot}\textit{\mathbf{F}} $ 在n上的投影，方向旋量沿旋度的方向达到最大值，也就是旋度达到方向是方向旋量最大的方向。
...
设 $ \textit{\mathbf{F}}=P\textit{\mathbf{i}}+Q\textit{\mathbf{j}}+R\textit{\mathbf{k}} $ 是定义在区域 $ D\subset R^3 $ 中的一个向量场，若沿 $\textit{D}$ 中的任意一条曲线 $ \wideparen{AB} $ 的积分

$$
\int_{\wideparen{AB}}\textit{\mathbf{F}}\cdot \mathrm{d}\textit{\mathbf{r}}=\int_{\wideparen{AB}}P\mathrm{d}x+Q\mathrm{d}y+R\mathrm{d}z
$$

只与曲线的起点\textit{A}和终点 $\textit{B}$ 有关，而与曲线 $ \wideparen{AB} $ 的路径无关，则称向量场\textit{\mathbf{F}}在 $\textit{D}$ 内是一个保守场。

设D是一个线性单连通区域，而 $ \textit{\mathbf{F}}=P\textit{\mathbf{i}}+Q\textit{\mathbf{j}}+R\textit{\mathbf{k}} $ 是定义在区域 $ D $ 中的一个向量场，其中P,Q,R在D内有一阶连续偏导数，那么下列三个条件是等价的


- $ \textit{\mathbf{F}} $  在D内是保守场\\
- $ \textit{\mathbf{F}} $  在D内沿任意闭曲线 $ \Gamma $ 的环量为零，即 $ \int_{\wideparen{AB}}\textit{\mathbf{F}}\cdot \mathrm{d}\textit{\mathbf{r}}=0 $ \\
- $ \textit{\mathbf{F}} $  是无旋场，即 $ \mathrm{rot}\textit{\mathbf{F}}=0 $ 


与保守场相关的是有势场，如果对于向量场\textit{\mathbf{F}}，存在一个函数\textit{f}使得其负梯度恰好是\textit{\mathbf{F}}，也即

$$
\textit{\mathbf{F}}=-\mathrm{grad}f
$$
