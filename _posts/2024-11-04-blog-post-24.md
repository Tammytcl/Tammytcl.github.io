...
$$\int_{\overset{AB}}f(m)\mathrm{d}s=\int_{\overset{BA}}f(m)\mathrm{d}s$$
...
我们用分点 $ A=A_{0},A_{1},\cdots ,A_{n}=B $ 将曲线 $ \overset{AB} $ 任意分割成 $ \textit{n} $ 段小弧

$$\overset{A_{i-1}A_{i}}\quad(i=1,2,\cdots,n)$$

设第i段的弧长为 $ \Delta s_{i} $ ，当 $ \Delta s_{i} $ 很小时，\mathit{\mathbf{F}}在 $ \overset{A_{i-1}A_{i}} $ 上的变化不大，可近似的看作为常力 $ \mathit{\mathbf{F}}(\xi_{i},\eta_{i}) $ ，其中 $ (\xi_{i},\eta_{i}) $ 为弧段 $ \overset{A_{i-1}A_{i}} $ 上任取的一点，同时可将指点的运动轨迹 $ \overset{A_{i-1}A_{i}} $ 近似的看作从 $ A_{i-1} $ 到 $ A_{i} $ 的一条直线，于是\mathit{\mathbf{F}}在这段上做的功 $ \Delta W_{i} $ ，近似为
...
令 $ \lambda = \max\limits_{1\leq i \leq n}{\Delta s_{i}} $ ，我们定义变力\mathit{\mathbf{F}}沿 $ \overset{AB} $ 所做的功为
...
$$\int_{\overset{AB}}P\mathrm{d}x+Q\mathrm{d}y \quad or \quad  \int_{\overset{AB}}\mathit{\mathbf{F}}(x,y)\cdot \mathrm{d}\mathit{\mathbf{r}}$$

其中 $ \mathrm{d}\mathit{\mathbf{r}}=(\mathrm{d}x,\mathrm{d}y) $ ，有向线段 $ \overset{AB} $ 称为积分路径

由定义看出，外力 $\textit{F}$ 沿曲线 $\textit{L}$ 从 $\textit{A}$ 到 $ $\textit{B}$ $ 所做的功可表示为第二类曲线积分

$$W=\int_{\overset{AB}}\mathit{\mathbf{F}}(x,y)\cdot \mathrm{d}\mathit{\mathbf{r}}$$
...
$$
\begin{align*}
\int_{\overset{AB}}P(x,y)\mathrm{d}x+Q(x,y)\mathrm{d}y=
...
$$\oint_{\overset{AB}}P(x,y)\mathrm{d}x+Q(x,y)\mathrm{d}y$$

的值只与起点 $\textit{A}$，终点 $\textit{B}$ 的坐标有关，而与连接 $\textit{A,B}$ 两点的积分路径无关。

### 命题
在区域 $\textit{D}$ 内任意选定两点 $\textit{A,B}$，曲线积分 $ \oint_{\overset{AB}}P(x,y)\mathrm{d}x+Q(x,y)\mathrm{d}y $ 在区域 $\textit{D}$ 内与路径无关的充分必要条件是：对于 $\textit{D}$ 内任意一条简单逐段光滑的闭合曲线 $\textit{C}$，沿 $\textit{C}$ 的曲线积分为零，即
...
根据这张图，我们可以把闭合曲线 $\textit{C}$ 分为 $ \overset{AnB} $ 和 $ \overset{AmB} $ 

$$\oint_{\overset{AnB}}P(x,y)\mathrm{d}x+Q(x,y)\mathrm{d}y=\oint_{\overset{AmB}}P(x,y)\mathrm{d}x+Q(x,y)\mathrm{d}y$$

移项得

$$
\begin{align*}
0=&\oint_{\overset{AmB}}P(x,y)\mathrm{d}x+Q(x,y)\mathrm{d}y-\oint_{\overset{AnB}}P(x,y)\mathrm{d}x+Q(x,y)\mathrm{d}y\\
=&\oint_{\overset{AmB}}P(x,y)\mathrm{d}x+Q(x,y)\mathrm{d}y+\oint_{\overset{BnA}}P(x,y)\mathrm{d}x+Q(x,y)\mathrm{d}y\\
=&\oint_{C^{+}}Pdx+Qdy
\end{align*}
$$
...
设函数 $ P(x,y),Q(x,y) $ 在单连通区域 $\textit{D}$ 内有连续的一阶偏导数，对任意两点 $ A,B\in D $ ，曲线积分 $ \oint_{\overset{AB}}P(x,y)\mathrm{d}x+Q(x,y)\mathrm{d}y $ 与路径无关的充要条件是： $ P(x,y)\mathrm{d}x+Q(x,y)\mathrm{d}y $ 恰是某个函数 $ u(x,y) $ 的全微分，此外，当 $ P(x,y)\mathrm{d}x+Q(x,y)\mathrm{d}y $ 是 $ u(x,y) $ 的全微分时，有

$$\oint_{\overset{AB}}P(x,y)\mathrm{d}x+Q(x,y)\mathrm{d}y=\int_{A}^{B}\mathrm{d}u=u(B)-u(A)$$
...
设 $ \mathit{\mathbf{F}}=P\mathit{\mathbf{i}}+Q\mathit{\mathbf{j}}+R\mathit{\mathbf{k}} $ 是定义在区域 $ D\subset R^3 $ 中的一个向量场，若沿 $\textit{D}$ 中的任意一条曲线 $ \overset{AB} $ 的积分

$$
\int_{\overset{AB}}\mathit{\mathbf{F}}\cdot \mathrm{d}\mathit{\mathbf{r}}=\int_{\overset{AB}}P\mathrm{d}x+Q\mathrm{d}y+R\mathrm{d}z
$$

只与曲线的起点\textit{A}和终点 $\textit{B}$ 有关，而与曲线 $ \overset{AB} $ 的路径无关，则称向量场\mathit{\mathbf{F}}在 $\textit{D}$ 内是一个保守场。
...
- $ \mathit{\mathbf{F}} $  在D内沿任意闭曲线 $ \Gamma $ 的环量为零，即 $ \int_{\overset{AB}}\mathit{\mathbf{F}}\cdot \mathrm{d}\mathit{\mathbf{r}}=0 $ \\
